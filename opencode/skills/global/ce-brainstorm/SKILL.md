...
5. **Pipeline override.** When invoked from LFG or any `disable-model-invocation` context, force `OUTPUT_FORMAT=md` regardless of steps 1-4. Downstream consumers (`ce-plan`, `goal-to-tickets`) parse markdown reliably; HTML in pipeline runs is unnecessary friction.
