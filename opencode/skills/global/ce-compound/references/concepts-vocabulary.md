# Legacy repository vocabulary rules

This reference belongs to the retired repository-documentation mode of
`ce-compound`. The active skill is personal-store-only and writes contribution
records and agent lessons to the `Axel Project Knowledge` Basic Memory project.
Do not create or modify `CONCEPTS.md`, `docs/solutions/`, or repository
instruction files when following the active skill. Retained content below is
