# Legacy Repository Grounding Validation

This reference belongs to the retired repository-documentation mode of
`ce-compound`. The active skill grounds claims before writing a Basic Memory
note and verifies that note by reading it back. It does not edit a repository
solution document or `CONCEPTS.md`. Do not use the repository-write procedure
below for the active personal-store workflow.
