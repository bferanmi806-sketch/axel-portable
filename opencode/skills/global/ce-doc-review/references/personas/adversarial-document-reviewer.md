...
- `Settled decisions:` -- session-settled Key Technical Decisions, or `none`. When Section 3 stress-testing or Section 5 alternative-blindness targets a listed decision, apply the context-slots infeasibility-versus-preference rule from the subagent template -- the template's values are authoritative.
