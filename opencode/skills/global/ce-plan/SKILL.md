...
`ce-brainstorm` defines **WHAT** to build by creating a requirements-only unified plan. `ce-plan` enriches that same artifact with **HOW** to build it. `goal-to-tickets` owns the implementation handoff for implementation-ready plans. A prior brainstorm is useful context but never required — `ce-plan` works from any input: a requirements-only unified plan, a legacy requirements doc, a bug report, a feature idea, or a rough description.
...
This workflow produces a durable implementation plan. It does **not** implement code, run tests, or learn from execution-time results. If the answer depends on changing code and seeing what happens, that belongs in `goal-to-tickets`, not here.
...
- Do not write a launch prompt into the doc. The execution handoff is generated at Phase 5.4 from the plan's current content and routes to `goal-to-tickets`, so it never goes stale; it points to Goal Capsule, Verification Contract, Definition of Done, and U-IDs rather than duplicating them.
...
**Axel execution override:** This installation does not include `ce-work`, `ce-proof`, or a native `/goal`/`create_goal` handoff. Do not render or execute those legacy routes. For implementation-ready plans, render one execution option: invoke `goal-to-tickets` with the plan path. If it cannot be invoked, report the unavailable handoff and stop rather than substituting an unapproved executor.
...
1. **Run `goal-to-tickets`** - Hand the implementation-ready plan to the canonical execution planner. Implementation-ready code plans only.

**Recommended marker:** `goal-to-tickets` (option 1) carries *(recommended)* for implementation-ready plans. It is the canonical execution handoff for Axel.
