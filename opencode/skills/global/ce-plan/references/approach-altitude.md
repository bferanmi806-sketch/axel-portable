...
**Save for later.** Persist the approach-plan to `docs/plans/` so it survives. If the deliverable is non-code, write the marker (`execution: knowledge-work`, see `references/plan-sections.md`) at persist time so a later `goal-to-tickets` invocation on the saved plan routes to the appropriate execution path. Do not label an approach-plan as `artifact_contract: ce-unified-plan/v1` unless it also contains Product Contract, Planning Contract, Implementation Units, Verification Contract, and Definition of Done for a software implementation. A plan-for-a-plan is usually not suitable for immediate execution; its job is to guide a later planning or knowledge-work pass. Offer to deepen it. Keep the plan **agent-agnostic** so any agent can execute it later.

**Execute now -- code deliverable.** The approach-plan's job is done; continue into the normal `ce-plan` flow (Phase 0.1b onward) to produce the implementation plan, then hand off to `goal-to-tickets` for the code. `ce-plan` never writes the code itself.

**Execute now -- non-code deliverable.** This is the knowledge-work path, so it routes to `goal-to-tickets` with the `execution: knowledge-work` marker:
...
3. Invoke `goal-to-tickets` with the plan path using the host's normal skill-invocation mechanism. Do not substitute a generic Task, Agent, or subagent. If it cannot be invoked, say execution did not start and print a copyable handoff prompt instructing the next agent to invoke `goal-to-tickets` with the plan path.

`ce-plan` itself does not execute the deliverable in any path — it produces the approach-plan and hands off. The portable plan is also runnable by any other agent without relying on a CE-specific execution skill.
