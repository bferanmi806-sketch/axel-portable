...
**Axel routing override:** The selected installation does not include `ce-work`, `ce-proof`, or a native `/goal`/`create_goal` handoff. Do not render or execute those legacy options. For implementation-ready plans, the sole execution handoff is `goal-to-tickets` with the plan path; if it cannot be invoked, report that execution did not start and stop.
...
**Options:**
1. **Run `goal-to-tickets`** - Hand the implementation-ready plan to the canonical execution planner. Show only for `artifact_readiness: implementation-ready` plus `execution: code`; universal-planning, answer-seeking, approach-plan, and requirements-only artifacts keep their own handoff/checkpoint behavior.

**Recommended marker:** `goal-to-tickets` (option 1) carries *(recommended)*. It is the canonical execution entry point for Axel.
...
**Option 5 format-keyed label.** Under exclusive output mode, the plan exists as exactly one artifact — `.md` or `.html`, never both. Render the option 5 label matching the produced format. Proof is unavailable in this installation; the browser option applies to HTML runs. The single implementation handoff remains available in both modes only when the artifact is implementation-ready code — `goal-to-tickets` receives the plan path and the handoff is generated from the current artifact.
...
**Hide `Decide on the review's open items` (option 3) when no actionable findings remain or doc review was skipped.** Show this option only when the headless envelope reports `proposed_fixes_count + decisions_count > 0` — i.e., at least one `gated_auto` or `manual` finding at confidence anchor `75` or `100`. Drop the option in any other case, including FYI-only state. FYI observations (anchor `50`) do not enter `ce-doc-review`'s interactive routing question or walkthrough — that flow is gated to actionable findings — so a `Decide on the review's open items` option that only has FYIs to show is a dead-end. Always renumber the visible options 1-N for display. An implementation-ready plan shows `goal-to-tickets`, Create Issue, and the format-appropriate review/share option; a requirements-only plan hides the execution option. The summary line above the menu still names the FYI count when present.

**Cross-skill invocation rule:** Invoke `goal-to-tickets`, `ce-doc-review`, and `ce-proof` using the host's normal skill-invocation mechanism. Do not substitute a generic Task, Agent, or subagent; the invoked skill may still dispatch its own subagents according to its protocol.
...
- **Create Issue** -> Follow the Issue Creation section below, then offer to proceed with `goal-to-tickets`.
...
- Ask whether to proceed with `goal-to-tickets` using the platform's blocking question tool
