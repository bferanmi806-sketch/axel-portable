...
SOURCE = "corpus"
SUPPORTED_SUFFIXES = {
    ".md", ".txt", ".pdf", ".docx", ".pptx", ".xlsx", ".html", ".htm",
    ".csv", ".json", ".xml", ".epub",
}
...
CACHE_SCHEMA_VERSION = "last30days-corpus-cache/v3"
    ...
    for root_index, root in enumerate(readable_roots):
        ...
        for path in _iter_files(root, notes=notes):
            ...
            if (
                ...
                if path.suffix.lower() == ".pdf" and not pdf_available and not markitdown_available:
                    if not pdf_unavailable_noted:
                        notes.append("Skipped PDF files because neither MarkItDown nor pdftotext is available")
                        pdf_unavailable_noted = True
                    continue
                try:
                    text = _extract_text(
                        path,
                        pdftotext=pdf_available,
    ...
    *,
    ...
    if markitdown:
        try:
            ...
            return (result.text_content or "")[:MAX_TEXT_CHARS]
        except Exception:
            # Keep the existing PDF extractor and plain-text path as fallbacks.
            pass
