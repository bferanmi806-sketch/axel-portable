...
When running in Axel OpenCode, use the connected Paseo browser tools for navigation, snapshots, screenshots, resizing, interaction, and console inspection. Treat those tools as the in-app browser surface described below. Use the local preview URL exposed by the current project rather than `terminal.local`. This is a host compatibility mapping only; preserve the capture-first and browser-verification requirements.
