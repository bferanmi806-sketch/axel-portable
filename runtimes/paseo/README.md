...
source checkout. `config.template.json` preserves its non-secret behavior,
including a sanitized default agent profile, while replacing
workstation-specific paths and the embedded prompt with a marker. The exact
prompt is preserved in `identity/AXEL.md`.
