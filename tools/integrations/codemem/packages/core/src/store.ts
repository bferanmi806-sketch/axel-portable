...
import { and, desc, eq, gt, inArray, isNotNull, isNull, lt, lte, or, type SQL, sql } from "drizzle-orm";
		...
		const baseQuery = this.d
			...
			.where(
				and(
					...
					...(untilTsWallMs == null
						? []
						: [or(isNull(schema.rawEvents.ts_wall_ms), lte(schema.rawEvents.ts_wall_ms, untilTsWallMs))]),
				),
			)
