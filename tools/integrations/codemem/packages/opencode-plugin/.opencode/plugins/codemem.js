...
const resolveWorkingDirectory = ({ worktree, directory }) => {
  const candidateWorktree = typeof worktree === "string" ? worktree.trim() : "";
  const candidateDirectory = typeof directory === "string" ? directory.trim() : "";

  // OpenCode can expose its virtual root as `worktree` for global projects;
  // keep the real directory so project identity and raw-event provenance survive.
  if (
    candidateWorktree
    && !(isFilesystemRoot(candidateWorktree) && candidateDirectory && !isFilesystemRoot(candidateDirectory))
  ) {
    return candidateWorktree;
  }
  return candidateDirectory || candidateWorktree || process.cwd();
};
...
const resolveProjectName = (project, cwd) =>
  ...
  normalizeProjectLabel(project?.worktree) ||
  normalizeProjectLabel(project?.vcsDir) ||
  normalizeProjectLabel(project?.directory) ||
...
export const OpencodeMemPlugin = async ({
  ...
  const cwd = resolveWorkingDirectory({ worktree, directory });
